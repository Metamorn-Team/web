  ...
  return (
    <header className="fixed top-0 left-0 w-full min-h-14 z-40 flex flex-wrap justify-between items-center px-4 sm:px-6 sm:py-3 bg-[#fdf8ef] border-b border-[#bfae96] shadow-[4px_4px_0_#8c7a5c]">
      ...
      <div className="relative flex gap-2 sm:gap-3" ref={menuRef}>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full mt-2 w-44 bg-[#fdf8ef] border border-[#bfae96] shadow-[4px_4px_0_#8c7a5c] p-2 flex flex-col gap-2 text-sm text-[#3d2c1b] animate-fadeIn rounded-[6px] sm:static sm:mt-2">
            {isLogined ? (
              <DropdownItem
                icon={<FiLogOut />}
                label="로그아웃"
